...
func main() {
	...
	totalSizeCollecion := sumDir(&filesystem)
	fmt.Println(findDirToDelete(totalSizeCollecion))
}
...
func sumDir(dir *Directory) []int {
	totalSize := 0
	totalSizeCollection := []int{}
	totalSize += dir.totalFileSize
	for _, d := range dir.children {
		t := sumDir(d)
		totalSizeCollection = append(totalSizeCollection, t...)
		totalSize += d.totalSize
	}

	dir.totalSize = totalSize

	return append(totalSizeCollection, dir.totalSize)
}

func findDirToDelete(totalSizeCollection []int) int {
	freeSpace := 70000000 - totalSizeCollection[len(totalSizeCollection)-1]

	sort.Ints(totalSizeCollection)

	for _, v := range totalSizeCollection[:len(totalSizeCollection)-1] {
		if freeSpace+v >= 30000000 {
			return v
		}
	}
