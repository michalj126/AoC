		...
		if len(line) > 0 {
			...
			if line[0:1] == "S" {
				...
				for _, v := range items {
					...
					round.monkeys[len(round.monkeys)-1].items = append(round.monkeys[len(round.monkeys)-1].items, vInt)
				}
			}
			...
			if line[0:1] == "O" {
				...
				if operation[0] != "old" {
					...
					round.monkeys[len(round.monkeys)-1].operation.x = xInt
				}
				...
				if operation[2] != "old" {
					...
					round.monkeys[len(round.monkeys)-1].operation.y = yInt
				}
			...
			if line[0:1] == "T" {
				...
				round.monkeys[len(round.monkeys)-1].test.divisibleBy = divisibleBy
			}
