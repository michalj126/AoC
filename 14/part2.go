		...
		for {
			...
			if cave.caveMap[yTemp+1][xTemp] == "#" || cave.caveMap[yTemp+1][xTemp] == "o" {
				if cave.caveMap[yTemp+1][xTemp-1] != "#" && cave.caveMap[yTemp+1][xTemp-1] != "o" {
					...
					if xTemp == source.x-cave.minX && yTemp == source.y {
						end = true
					}
					break
				}
...
func increaseMapLeftSide() {

	for i := 0; i < 1; i++ {
		for i := range cave.caveMap {
			cave.caveMap[i] = append([]string{"."}, cave.caveMap[i]...)
		}

		cave.caveMap[len(cave.caveMap)-1][0] = "#"
	}

	source.x += 1
}

func increaseMapRightSide() {
	for i := 0; i < 1; i++ {
		for i := range cave.caveMap {
			cave.caveMap[i] = append(cave.caveMap[i], ".")
		}

		cave.caveMap[len(cave.caveMap)-1][len(cave.caveMap[0])-1] = "#"
	}
}
...
func createMap() {
	...
	for i := 0; i < len(cave.caveMap[0]); i++ {
		cave.caveMap[len(cave.caveMap)-1] = append(cave.caveMap[len(cave.caveMap)-1], ".")
	}
	...
	cave.caveMap[0][source.x-cave.minX] = "+"
}
